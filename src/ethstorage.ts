...
export class EthStorage {
    ...
    async estimateCost(key: string, data: Uint8Array): Promise<CostEstimate> {
        ...
        const contract = new ethers.Contract(this.contractAddr, EthStorageAbi, this.wallet) as any;
    ...
    async read(key: string): Promise<Uint8Array> {
        ...
        const data = await contract.get(hexKey, DecodeType.OptimismCompact, 0, size, {
            from: this.wallet.address
        });
